  ...
  for (const key of groupKeys) {
    ...
    for (const p of state.groups[key]) {
      ...
      let moveBtns = '';
        ...
        + '<div>' + p + (_editMode ? '<button class="btn admin-only btn-secondary" style="padding:1px 5px;font-size:.65rem;margin-left:4px;" onclick="promptRename(' + idx + ')">✏️</button>' : '') + (m ? '<br><span class="text-muted" style="font-size:.75rem;">' + m.a + ' & ' + m.b + '</span>' : '') + '</div>'
